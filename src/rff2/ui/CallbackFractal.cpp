...
namespace merutilm::rff2 {
    const std::function<void(SettingsMenu &, RenderScene &)> CallbackFractal::REFERENCE =
        ...
        *centerPtr = {calc.center.real.to_string(), calc.center.imag.to_string()};
        ...
        window->registerTextInput<float>(
            ...
            L"Sets the rotation angle in degrees.");

        window->registerSectionHeader(L"Reference Cache");
        window->registerRadioButtonInput<FrtReuseReferenceMethod>(
            L"Reuse Reference", &calc.reuseReferenceMethod, Callback::NOTHING, L"Reuse Reference method",
            L"Sets the reuse reference method.");
        window->registerTextInput<uint32_t>(
            L"Reference Compression Criteria", &calc.referenceCompAttribute.compressCriteria,
            Unparser::U_LONG, Parser::U_LONG, ValidCondition::ALL_U_LONG, Callback::NOTHING,
        ...
        window->registerTextInput<uint8_t>(
            L"Reference Compression Threshold", &calc.referenceCompAttribute.compressionThresholdPower,
            Unparser::U_CHAR, Parser::U_CHAR, ValidCondition::ALL_U_CHAR, Callback::NOTHING,
        ...
        window->registerCheckboxInput(
            L"Disable Compressor Normalization", &calc.referenceCompAttribute.noCompressorNormalization,
            Callback::NOTHING, L"Disable Compressor Normalization",
    ...
    const std::function<void(SettingsMenu &, RenderScene &)> CallbackFractal::ITERATIONS =
        [](SettingsMenu &settingsMenu, RenderScene &scene) {
            ...
            window->registerTextInput<uint64_t>(
                L"Max Iteration", &calc.maxIteration, Unparser::U_LONG_LONG, Parser::U_LONG_LONG,
                [](const uint64_t &v) { return v >= 1; }, Callback::NOTHING, L"Set Max Iteration",
                L"Set maximum iteration. It is disabled when Automatic Iterations is enabled.");
            window->registerTextInput<uint16_t>(
                L"Auto Iteration Multiplier", &calc.autoIterationMultiplier, Unparser::U_SHORT,
                Parser::U_SHORT, [](const uint16_t &v) { return v >= 1; }, Callback::NOTHING,
                L"Set Auto Iteration Multiplier",
                L"Set auto iteration multiplier. It is disabled when Automatic Iterations is disabled.");

            window->registerTextInput<float>(
                L"Bailout", &calc.bailout, Unparser::FLOAT_SCIENTIFIC, Parser::FLOAT,
                [](const float &v) { return v >= 2 && v <= 1e38f; }, Callback::NOTHING, L"Set Bailout",
    ...
    const std::function<void(SettingsMenu &, RenderScene &)> CallbackFractal::MPA =
        ...
        auto &[minSkipReference, maxMultiplierBetweenLevel, epsilonPower, mpaSelectionMethod,
               mpaCompressionMethod] = scene.getAttribute().fractal.mpaAttribute;
        auto window = std::make_unique<SettingsWindow>(L"MP-Approximation");
        window->registerSectionHeader(L"Skip Table", false);
        window->registerTextInput<uint16_t>(
            L"Min Skip Reference", &minSkipReference, Unparser::U_SHORT, Parser::U_SHORT,
        ...
        window->registerTextInput<float>(
            L"Precision Level", &epsilonPower, Unparser::floatFixed(1), Parser::FLOAT,
            [](const float &v) { return v >= -15.0f && v <= -3.0f; }, Callback::NOTHING, L"Precision Level",
        ...
        window->registerRadioButtonInput<FrtMPASelectionMethod>(
            L"Selection Method", &mpaSelectionMethod, Callback::NOTHING, L"Selection Method",
            L"The first target PA is always the front element.");

        window->registerRadioButtonInput<FrtMPACompressionMethod>(
            L"Compression Method", &mpaCompressionMethod, Callback::NOTHING, L"Compression Method",
    ...
    const std::function<bool *(RenderScene &, bool)> CallbackFractal::ABSOLUTE_ITERATION_MODE =
        [](RenderScene &scene, bool) { return &scene.getAttribute().fractal.absoluteIterationMode; };
    ...
    const std::function<void(SettingsMenu &, RenderScene &)> CallbackFractal::FORMULA =
        ...
        window->registerRadioButtonInput<FractalFormulaType>(
            L"Formula Type", &calc.formulaType,
            [&scene] {
            ...
            L"Formula Type", L"Mandelbrot: Standard z^2+c\nCustom: Use custom formula string");

        window->registerSectionHeader(L"Custom Expression");
        auto formulaPtr = std::make_shared<std::string>(calc.customFormula);
        const HWND formulaField = window->registerTextInput<std::string>(
            L"Custom Formula", formulaPtr.get(), Unparser::STRING, Parser::STRING,
            ...
            L"Custom Formula", L"Enter custom formula (see syntax below)");
        ...
        window->registerInsertChips(
            formulaField, {L"abs",  L"sqrt", L"exp",   L"log",   L"sin",  L"cos",   L"tan",  L"asin", L"acos",
                           L"atan", L"sinh", L"cosh",  L"tanh",  L"real", L"imag",  L"conj", L"arg",  L"norm",
                           L"rabs", L"iabs", L"riabs", L"floor", L"ceil", L"round", L"sign"},
            false);

        // Full-formula examples: clicking one replaces the whole field.
        window->registerStaticText(L"Examples (click to replace):");
        window->registerInsertChips(formulaField, {L"z^2+c", L"z^3+c", L"riabs(z)^2+conj(c)", L"conj(z)^2+c"},
                                    true);

        // Display supported syntax directly in the window
        window->registerStaticText(L"=== Supported Syntax ===\n"
    ...
    const std::function<void(SettingsMenu &, RenderScene &)> CallbackFractal::PROJECTION =
        ...
        window->registerRadioButtonInput<FrtProjectionMethod>(
            L"Projection", &calc.projectionMethod, [&scene] { scene.getRequests().requestRecompute(); },
            L"Projection",
        ...
        window->registerRadioButtonInput<FrtPanoramaLayout>(
            L"Layout", &calc.panoramaLayout, [&scene] { scene.getRequests().requestRecompute(); }, L"Layout",
        ...
        window->registerTextInput<float>(
            L"Pitch", &calc.panoramaPitch, angleUnparser, angleParser,
            [](const float &v) { return v >= -90.0f && v <= 90.0f; },
            [&scene] { scene.getRequests().requestRecompute(); }, L"Pitch",
            ...
            1.0);
        window->registerTextInput<float>(
            L"Field of View", &calc.panoramaFov, angleUnparser, angleParser,
            [](const float &v) { return v >= 1.0f && v <= 179.0f; },
            [&scene] { scene.getRequests().requestRecompute(); }, L"Field of View",
        ...
        window->registerStaticText(
            L"Yaw is the Rotation of the Reference panel, and dragging the canvas moves it.");

        window->registerSectionHeader(L"Distance");
        window->registerTextInput<float>(
            L"Panorama Range", &calc.panoramaRange, Unparser::floatFixed(1), Parser::FLOAT,
            [](const float &v) { return v >= 0.0f && v <= 6.0f; },
            [&scene] { scene.getRequests().requestRecompute(); }, L"Panorama Range",
