...
namespace merutilm::vkh {
    void GeneralGraphicsPipelineConfigurator::configure() {
        ...
        vertexBuffer = factory::create<VertexBuffer>(wc.core, std::move(vertexData),
                                                     BufferLock::LOCK_UNLOCK, true);
        indexBuffer = factory::create<IndexBuffer>(wc.core, std::move(indexData),
                                                   BufferLock::LOCK_UNLOCK, true);

        pipeline = factory::create<GraphicsPipeline>(wc, pipelineLayout, *vertexBuffer, *indexBuffer,
