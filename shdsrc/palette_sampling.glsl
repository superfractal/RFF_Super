...
#ifndef PALETTE_EXTERNAL_ITERATION
...
layout (set = PALETTE_DATA_SET, binding = 0) buffer PaletteSSBO {
    vec4 interval;
    double offset;
    uint size;
...
layout (set = PALETTE_TIME_SET, binding = 0) uniform TimeUBO {
    float time;
...
layout (set = PALETTE_TEXTURE_SET, binding = 0) uniform sampler2D exterior_texture_0;
layout (set = PALETTE_TEXTURE_SET, binding = 1) uniform sampler2D exterior_texture_1;
layout (set = PALETTE_TEXTURE_SET, binding = 2) uniform sampler2D exterior_texture_2;
layout (set = PALETTE_TEXTURE_SET, binding = 3) uniform sampler2D exterior_texture_3;

layout (set = PALETTE_TEXTURE_SET, binding = 4) uniform TextureUBO {
    uint enabled_0;
    uint uv_mode_0;
    uint blend_mode_0;
    float opacity_0;
...
double smooth_iteration(double iteration) {
    switch (smoothing_word() & 0xFFu) {
        ...
        case REVERSED: return floor(iteration) + (1.0 - mod(iteration, 1));
...
double cbrt_d(double x) {
    ...
    double seed = ldexp(mantissa, exponent - 3 * scale);
    double y = ldexp(double(pow(float(seed), 1.0 / 3.0)), scale);
...
vec4 blend_palette(vec4 cc, vec4 nc, float t) {
    // Bit 14 selects Linear RGB without shifting legacy modes; IEC transfer/project GPL code, see NOTICE.
    if ((smoothing_word() & (1u << 14)) != 0u) {
        return vec4(linear_to_srgb(mix(srgb_to_linear(cc.rgb), srgb_to_linear(nc.rgb), t)),
                    mix(cc.a, nc.a, t));
    }
    ...
    return vec4(clamp(linear_to_srgb(oklab_to_linear(mix(a, b, t))), 0.0, 1.0),
                mix(cc.a, nc.a, t));
}
...
double static_ratio(double iteration, double interval, double inv_interval) {
    return mod(coloring_curve(div_r(smooth_iteration(iteration), interval, inv_interval))
               + palette_attr.offset, 1);
}
...
double animation_offset_iterations(vec2 rawCoord) {
    ...
    if (animation_mode() == ANIMATION_PSYCHEDELIC) {
        ...
        float r = length(p);
        float a = p.x == 0.0 && p.y == 0.0 ? 0.0 : atan(p.y, p.x);
        ...
        float liquid = sin((p.x * cos(t) + p.y * sin(t)) * s * float(TWO_PI)
                           + sin((p.y - p.x) * s * 3.0 + t));
        float pulse = 0.75 + 0.25 * sin(t * float(TWO_PI) * 0.25);
        offset_iterations += double((radial * 0.4 + spiral * 0.4 + liquid * 0.2)
                                    * pulse * palette_attr.animation_flow_amount);
    }
    return offset_iterations;
}

vec4 get_color_channel(double iteration, double interval, double inv_interval,
                       bool animate, double anim_iters, double warp_iters, bool unlined) {
    ...
    double timed_offset_ratio = palette_attr.offset - time_term
        + div_r(warp_iters, interval, inv_interval);
    ...
    double iter_ratio = biased_ratio(mod(
        coloring_curve(div_r(smooth_iteration(iteration), interval, inv_interval)), 1));
    double palette_offset_ratio = mod(iter_ratio + timed_offset_ratio, 1);
    if (isnan(palette_offset_ratio) || isinf(palette_offset_ratio)) {
        return palette_attr.mandelbrot_color;
    }
    if (palette_offset_ratio >= 1.0) {
        palette_offset_ratio = 0.0;
    }
    uint base = unlined ? palette_attr.size : 0u;
    uint size = unlined ? uint(palette_attr.palette.length()) - base : palette_attr.size;
    double palette_offset = palette_offset_ratio * double(size);
    ...
    uint cpl = min(uint(palette_offset_ratio * size), size - 1u);
    uint npl = (cpl + 1) % size;

    vec4 cc = palette_attr.palette[base + cpl];
    vec4 nc = palette_attr.palette[base + npl];

    return blend_palette(cc, nc, palette_offset_decimal);
}

vec4 get_color(double iteration, bool animate, double anim_iters, double warp_iters, bool unlined) {
...
vec4 get_color(double iteration, bool animate, double anim_iters, double warp_iters) {
    return get_color(iteration, animate, anim_iters, warp_iters, ordered_layers() && uint(palette_attr.palette.length()) > palette_attr.size);
}
...
vec2 iteration_gradient(ivec2 c, double center) {
    ...
    bool lv = c.x > 0 && l > 0 && l < mv;
    bool rv = c.x + 1 < extent.x && r > 0 && r < mv;
    bool dv = c.y > 0 && d > 0 && d < mv;
    bool uv = c.y + 1 < extent.y && u > 0 && u < mv;
...
vec2 decor_uv(double iteration, vec2 coord, vec2 band_grad, double anim_iters, double warp_iters,
    ...
    switch (uv_mode) {
        ...
        default: {
            vec2 p = screen - 0.5;
            base = vec2(decor_cycle_ratio(iteration, su, anim_iters, period, palette_follow, warp_iters),
                        ((p.x == 0.0 && p.y == 0.0 ? 0.0 : atan(p.y, p.x)) / float(TWO_PI) + 0.5) * angular_repeats(sv));
            break;
        }
    }
    return base + scroll;
...
vec4 apply_texture(vec4 base, double iteration, vec2 coord, vec2 band_grad, double anim_iters,
                   double warp_iters) {
    for (int i = 0; i < TEXTURE_LAYERS; i++) {
        if (ordered_layers() && !layer_is(2 + i)) continue;
        DecorLayer l = texture_layer(i);
        if (l.enabled == 0u || l.opacity <= 0.0) {
            continue;
        }
...
vec4 apply_pattern(vec4 base, double iteration, vec2 coord, vec2 band_grad, double anim_iters,
                   double warp_iters) {
    for (int i = 0; i < PATTERN_LAYERS; i++) {
        if (ordered_layers() && !layer_is(6 + i)) continue;
        PatternLayer p = pattern_layer(i);
        if (p.enabled == 0u || p.opacity <= 0.0) {
            continue;
        }
...
double get_palette_iteration(uvec2 iterCoord) {
